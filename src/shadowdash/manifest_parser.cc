...
void manifesta() {
  ...
  auto compile = rule({ bind("command", "g++ -O3 -c $in -o $out"_l),
                        bind("pool", "heavy_object_pool"_v) });
  rules.push_back(compile);

  // Define another rule
  auto link = rule({ bind("command", "g++ $in -o $out"_l) });
  rules.push_back(link);

  // Define builds with these rules
  auto build_c = build(list{ str{ "hello.o"_l } },   // Outputs
                       {},                           // Implicit Outputs
                       compile,                      // Rule
  ...
  auto build_l = build(list{ str{ "hello"_l } }, {}, link,
                       list{ str{ "hello.o"_l } }, {}, {}, {});
...
extern "C" void InitializeManifest() {
  manifesta();  // Initializes pools, rules, and builds
}
